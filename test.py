...
class SumoNet:
    ...
    def generate_OD_by_Gaussian(self, flow_mean=300, flow_sd=100):
        print ("Generating random traffics by O-D ...")
        if self.Conn_ == {}:
            raise("Please get network connectivity beforehand!")
        if flow_mean < flow_sd:
            raise("The standard deviation of flows must be greater than the mean flow!")
    ...
    def generate_route_file(self, multiclass=False, file='data/test.rou.xml'):
        print ("Building route xml file ...")
        if self.OD is None:
            raise("OD Matrix must be defined beforehand!")
        optParser = sumolib.options.ArgumentParser()
        options = optParser.parse_args()
        options.routefile = file
...
if __name__ == "__main__":
    ...
    us101_net = SumoNet("data/us101.net.xml")
