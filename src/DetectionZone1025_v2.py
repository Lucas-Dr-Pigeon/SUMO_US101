...
class detectionZone:
    ...
    def get_last_traffic_states(self, subscriptions, timespan):
        tresDict = self.CONCATE_VEHICLE_SUB(subscriptions[-timespan:])
        tresArray = np.asarray([ list(tresDict[tresID].values()) for tresID in tresDict ] )
        totalTresDistance = np.sum(tresArray[:,0])/1000
        totalTresTime = np.sum(tresArray[:,1])/3600
        zoneDensity = totalTresTime / (self.zoneLength/1000) / (timespan/3600)
        zoneFlow = totalTresDistance / (self.zoneLength/1000) / (timespan/3600)
        return zoneDensity, zoneFlow, zoneFlow/zoneDensity
